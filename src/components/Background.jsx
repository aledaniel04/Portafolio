...
const AnimatedBackground = () => {
  const blobRefs = useRef([]) // Array de referencias a los elementos
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      blobRefs.current.forEach((blob, index) => {
        if (blob) {  // Verificamos si el elemento no es null
          const initialPos = initialPositions[index]

          // Calculando el movimiento en ambas direcciones X y Y
          const xOffset = Math.sin(newScroll / 100 + index * 0.5) * 340 // Movimiento horizontal
          const yOffset = Math.cos(newScroll / 100 + index * 0.5) * 40 // Movimiento vertical
  ...
  return (
    <div className="fixed inset-0 ">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#4f4f4f10_2px,transparent_2px),linear-gradient(to_bottom,#4f4f4f10_2px,transparent_2px)] bg-[size:25px_25px]"></div>
    </div>
  )
}
